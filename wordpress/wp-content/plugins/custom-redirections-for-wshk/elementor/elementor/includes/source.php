 ...
 */
class Source_Custom extends Source_Remote {
    
    static $templateWithJson=[];
	 ...
	 */
	public function get_items2( $args = [] ) {
    ...
    public function get_items( $args = [] ) {
        $templates=parent::get_items($args=[]);
        $myTemplate=$this->getMyTempaltes();
       
        return array_merge($myTemplate,$templates);
       
    }
    
    function getMyTempaltes(){
	    $myResponseTemplate=[];
        $myTemplates=dirname(__FILE__)."/../libraries/main.json";
        if(file_exists($myTemplates)){
            $data=@json_decode(file_get_contents($myTemplates));
            if(!empty($data) && !empty($data->templates) && is_array($data->templates)){
                foreach ($data->templates as $key=>$template) {
                    $myResponseTemplate[]=$this->get_template_item_info($key,$template);
                }
            }
        }
        return $myResponseTemplate;
        
    }
    function get_template_item_info($id,$json_row){
        $item=[];
        $item["template_id"]= "WSHK-".$id;
        $item["source"]= "remote";
        $item["type"]= "page";
        $item["subtype"]= "wshk";
        $item["title"]= $json_row->title;
        $item["thumbnail"]= plugins_url("/../templates/".$json_row->thumbnail,__FILE__);
        $item["url"]= $json_row->preview_url;
        $item["date"]= time();
        $item["author"]= "WSHK Plugin";
        $item["tags"]= ["wshk"];
        $item["isPro"]= false;
        $item["popularityIndex"]= 999;
        $item["trendIndex"]= 999;
        $item["favorite"]= false;
        self::$templateWithJson[$item["template_id"]]=dirname(__FILE__)."/../templates/".$json_row->json_data;
        return $item;
    }
	 ...
	 */
	public function get_data2( array $args, $context = 'display' ) {
    ...
    public function get_data( array $args, $context = 'display' ) {
        $isWSHK=strtoupper(substr($args['template_id'],0,5))=="WSHK-";
        if($isWSHK){
            $this->getMyTempaltes();
            if(!empty(self::$templateWithJson[$args['template_id']]) && file_exists(self::$templateWithJson[$args['template_id']])){
                $data=(array)json_decode(file_get_contents(self::$templateWithJson[$args['template_id']]),true);
                $data['content'] = $this->replace_elements_ids( $data['content'] );
                $data['content'] = $this->process_export_import_content( $data['content'], 'on_import' );
