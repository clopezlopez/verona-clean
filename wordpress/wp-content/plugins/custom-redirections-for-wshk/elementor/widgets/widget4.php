...
class Wshk_Widget_4 extends Widget_Base {
    ...
    $custompaymentredi = get_option('wshk_mipaymentsid');
		...
		$this->add_control(
			...
			[
				...
				'placeholder' => $custompaymentredi,
				'description' => __('If the user do click on the add payment method button, it will display the form to add a new redirecting to the same tab, its works too with the make default and delete buttons, just need to add the tab slug. Ex: #tab4.','woo-shortcodes-kit'),
	...
	protected function render() {
        ...
        if (!empty($settings['wshk_mipaymentsid']) ) {
            
            update_option( 'wshk_mipaymentsid', $settings['wshk_mipaymentsid'] );
            
        }else {
            
            $settings['wshk_mipaymentsid'] = get_option('wshk_mipaymentsid');
            
            update_option( $settings['wshk_mipaymentsid'], get_option('wshk_mipaymentsid') );
        }
