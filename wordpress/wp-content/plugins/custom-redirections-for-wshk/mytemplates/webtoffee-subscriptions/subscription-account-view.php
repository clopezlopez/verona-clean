...
class HF_Subscription_query extends WC_Query {
    ...
    public function init_query_vars() {
        ...
        if (!HF_Woocommerce_Subscription_Admin::is_woocommerce_prior_to('2.6')) {
    ...
    public function get_endpoint_title($endpoint) {
        ...
        switch ($endpoint) {
            case 'view-subscription':
                ...
                $title = ( $subscription ) ? sprintf(__('Subscription #%s', HF_Woocommerce_Subscription_Admin::TEXT_DOMAIN), $subscription->get_order_number()) : '';
                break;
            case 'subscriptions':
                $title = get_option(HF_Woocommerce_Subscription_Admin::$option_prefix . '_subscription_tab_title', 'Subscriptions');
                if (empty($title))
                    $title = __('Subscriptions', HF_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
                break;
            default:
                $title = '';
                break;
    ...
    public function add_menu_items($items) {
        ...
        $title = get_option(HF_Woocommerce_Subscription_Admin::$option_prefix . '_subscription_tab_text', 'Subscriptions');
        if (empty($title))
            $title = __('Subscriptions', HF_Woocommerce_Subscription_Admin::TEXT_DOMAIN);
    ...
    public function render_endpoint_content() {
        HF_Woocommerce_Subscription_Public::get_my_subscriptions_template();
    }
