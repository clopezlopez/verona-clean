...
if (strpos($_SERVER['REQUEST_URI'], '/livecanvas-wp/') !== false)	define("LC_SCRIPTS_VERSION", rand(0, 1000)); else define("LC_SCRIPTS_VERSION", "3.2.0");
...
if (lc_plugin_option_is_set("enable-dynamic-templating")):
	...
	if( !isset($_GET['lc_dynamic_template']) &&  (!is_admin() OR wp_doing_ajax())  && (!isset($_GET['post_type']) OR $_GET['post_type']!='lc_dynamic_template')) require("modules/templating-shortcodes.php");
endif;
...
function lc_cpts() {
	...
	register_post_type( 'lc_dynamic_template',
        array(
			...
			'description' => __('Dynamic Templates are useful for handling the WordPress single templates, archives, 404s...', 'livecanvas'),
            'has_archive' => false,
			'hierarchical' => false,
            'public' => false,
			'show_ui' => true,
